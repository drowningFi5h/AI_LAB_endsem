...
//     }
//   }
//   int **shifted_result = shift_rows(result_copy);
//   for (int i = 0; i < 4; i++)
//   {
//     for (int j = 0; j < 4; j++)
//     {
...
{
  int ***round_keys = (int ***)malloc(16 * sizeof(int **));
  for (int i = 0; i < 15; i++)
  {
    round_keys[i] = (int **)malloc(4 * sizeof(int *));
    for (int j = 0; j < 4; j++)
  ...
  int **copy_plain_text = (int **)malloc(4 * sizeof(int *));
  for (int i = 0; i < 4; i++)
  {
    copy_plain_text[i] = (int *)malloc(4 * sizeof(int));
  }
  for (int i = 0; i < 4; i++)
  {
    for (int j = 0; j < 4; j++)
    {
      copy_plain_text[i][j] = pt[i][j];
    }
  }
  int *col = (int *)malloc(4 * sizeof(int));
  for (int i = 0; i < 4; i++)
  {
    for (int j = 0; j < 4; j++)
    {
      copy_plain_text[i][j] ^= round_keys[0][i][j];
    }
  }
  // printf("After xor with key\n");
  // for (int i = 0; i < 4; i++)
  ...
  for (int r = 1; r < 14; r++)
  {
    ...
    for (int i = 0; i < 4; i++)
    {
      for (int j = 0; j < 4; j++)
      {
        copy_plain_text[i][j] = cal_sub(copy_plain_text[i][j]);
      }
    }
    ...
    // }
    // shift rows
    int copy_plain_text_copy[4][4];
    for (int i = 0; i < 4; i++)
    {
      for (int j = 0; j < 4; j++)
      {
        copy_plain_text_copy[i][j] = copy_plain_text[i][j];
      }
    }
    int **shifted_result = shift_rows(copy_plain_text_copy);
    for (int i = 0; i < 4; i++)
    {
      for (int j = 0; j < 4; j++)
      {
        copy_plain_text[i][j] = shifted_result[i][j];
      }
    }
    // printf("After shift rows\n");
    // for (int i = 0; i < 4; i++)
    ...
    for (int i = 0; i < 4; i++)
    {
      col[i] = copy_plain_text[i][0];
    }
    col = mix_col(col);
    for (int i = 0; i < 4; i++)
    {
      copy_plain_text[i][0] = col[i];
    }
    // printf("After mix col for col-1\n");
    // for (int i = 0; i < 4; i++)
    // {
    //   for (int j = 0; j < 4; j++)
    //   {
    ...
    for (int i = 0; i < 4; i++)
    {
      col[i] = copy_plain_text[i][1];
    }
    col = mix_col(col);
    for (int i = 0; i < 4; i++)
    {
      copy_plain_text[i][1] = col[i];
    }
    // printf("After mix col for col-2\n");
    // for (int i = 0; i < 4; i++)
    // {
    //   for (int j = 0; j < 4; j++)
    //   {
    ...
    for (int i = 0; i < 4; i++)
    {
      col[i] = copy_plain_text[i][2];
    }
    col = mix_col(col);
    for (int i = 0; i < 4; i++)
    {
      copy_plain_text[i][2] = col[i];
    }
    // printf("After mix col for col-3\n");
    // for (int i = 0; i < 4; i++)
    // {
    //   for (int j = 0; j < 4; j++)
    //   {
    ...
    for (int i = 0; i < 4; i++)
    {
      col[i] = copy_plain_text[i][3];
    }
    col = mix_col(col);
    for (int i = 0; i < 4; i++)
    {
      copy_plain_text[i][3] = col[i];
    }
    free(col);
    // printf("After mix col for col-4\n");
    // for (int i = 0; i < 4; i++)
    // {
    //   for (int j = 0; j < 4; j++)
    //   {
    ...
    for (int i = 0; i < 4; i++)
    {
      for (int j = 0; j < 4; j++)
      {
        copy_plain_text[i][j] ^= round_keys[r][i][j];
      }
    }
    // printf("After xor with key\n");
    // for (int i = 0; i < 4; i++)
  ...
  // }
  // shift rows
  int copy_plain_text_copy_last[4][4];
  for (int i = 0; i < 4; i++)
  {
    for (int j = 0; j < 4; j++)
    {
      copy_plain_text_copy_last[i][j] = copy_plain_text[i][j];
    }
  }
  int **shifted_result_last = shift_rows(copy_plain_text_copy_last);
  for (int i = 0; i < 4; i++)
  {
    for (int j = 0; j < 4; j++)
    {
      copy_plain_text[i][j] = shifted_result_last[i][j];
    }
  }
  free(shifted_result_last);
  // printf("After shift rows\n");
  // for (int i = 0; i < 4; i++)
  // {
  //   for (int j = 0; j < 4; j++)
  ...
  for (int i = 0; i < 4; i++)
  {
    for (int j = 0; j < 4; j++)
    {
      copy_plain_text[i][j] ^= round_keys[14][i][j];
    }
  }
  // printf("After last round\n");
  // for (int i = 0; i < 4; i++)
  // {
  //   for (int j = 0; j < 4; j++)
  //   {
...
int main()
{
  ...
  int ** cipher_text = (int **)malloc(4 * sizeof(int *));
  for (int i = 0; i < 4; i++)
  {
    cipher_text[i] = (int *)malloc(4 * sizeof(int));
  }
  cipher_text = aes_encryption(plaintext, key);
  printf("Cipher text: \n");
  for (int i = 0; i < 4; i++)
  {
    for (int j = 0; j < 4; j++)
