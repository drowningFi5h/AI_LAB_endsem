...
// Function to generate keys from the master key
void generate_keys(uint32_t master_key, uint16_t round_keys[5]) {
    for (int i = 0; i < 5; i++) {
        round_keys[i] = (master_key >> (16 - 4 * i)) & 0xFFFF;
    }
}
...
uint16_t encrypt(uint16_t plaintext, uint16_t round_keys[5]) {
    ...
    for (int i = 0; i < 4; i++) {
        state ^= round_keys[i];
        state = sbox_transform(state, 0);
        state = permute(state, 0);
    }
    state ^= round_keys[4];
    return state;
}
...
uint16_t decrypt(uint16_t ciphertext, uint16_t round_keys[5]) {
    ...
    state ^= round_keys[4];
    for (int i = 3; i >= 0; i--) {
        state = permute(state, 1);
        state = sbox_transform(state, 1);
        state ^= round_keys[i];
    }
    return state;
}
...
int main() {
    ...
    generate_keys(master_key, round_keys);
